...
def run():
    ...
    def add_roles_and_labels(filename, new_id, new_roles_and_labels):
        data = []
        if not os.path.exists(filename):
            new_data = {"id": new_id, "roles_and_labels": new_roles_and_labels}
            ...
            new_entry = {"id": new_id, "roles_and_labels": new_roles_and_labels}
            ...
            async def button_callback(interaction: discord.Interaction):
                ...
                if guild_role:
                    if guild_role in user.roles:
                        ...
                        await interaction.response.send_message(
                            "Role given!", ephemeral=True
                        )
                else:
                    await interaction.response.send_message(
                        "Role doesn't exist!", ephemeral=True
                    )
    ...
    class CatState(discord.ui.View):
        ...
        async def send_meow_message(self):
            random_days = [6, 5]  # random.sample(range(1, 7), 2)
            ...
            happy_gifs = [
                happy_gif.strip() for happy_gif in happy_gifs if happy_gif.strip()
            ]
            with open("angry_cat_gifs.txt", "r") as file:
                angry_gifs = file.readlines()
            angry_gifs = [
                angry_gif.strip() for angry_gif in angry_gifs if angry_gif.strip()
            ]
            ...
            if current_day in random_days:
                ...
                await bot.change_presence(
                    activity=discord.Game(name="Cat will meow today a lot!")
                )
                    ...
                    for _ in range(meow_frequency):
                        ...
                        if self.cat_hungry == 5:
                            ...
                            async def button_callback(
                                ...
                                self.cat_hungry = 1
                                self.meow_interval = random.randint(
                                    *interval_meow_range
                                )
                                if self.angry_state:
                                    self.angry_state = False
                                    await interaction.response.send_message(
                                        f"Meow :3 *He is happy now* \nTreat given by: {interaction.user.mention} \n{chosen_happy_gif}"
                                    )
                            ...
                            last_message = await channel.send(
                                f"Meow! *I think he wants a treat. Give him a treat!* \n{treat_gif}",
                                view=view,
                            )
                            await asyncio.sleep(disable_button_interval)
                            button.disabled = True
                            await last_message.edit(
                                content=last_message.content, view=view
                            )
        ...
        async def button_callback(self, interaction: discord.Interaction):
            ...
            if role:
                if role in user.roles:
                    await interaction.response.send_message(
                        "You have already accepted rules!", ephemeral=True
                    )
                    logger.info(
                        f"User {user} tried to accept rules but already has accepted rules!."
                    )
                else:
                    await interaction.user.add_roles(role)
                    await interaction.response.send_message(
                        "Rules Accepted! Have a nice stay!", ephemeral=True
                    )
    ...
    async def on_ready():
        ...
        try:
            ...
            logger.info("Synced application commands successfully.")
        ...
        if channel is None:
            try:
                channel = await bot.fetch_channel(ROLES_CHANNEL_ID)
                logger.info("Fetched channel %s for role buttons restore.", channel)
            except discord.HTTPException as fetch_error:
                logger.info(
                    f"Failed to fetch channel {ROLES_CHANNEL_ID}: {fetch_error}"
                )
                channel = None

        if channel and os.path.exists(roles_buttons_filename):
            async for message in channel.history(limit=None):
                view = RoleAssignView(
                    search_for_id_and_assign(roles_buttons_filename, message.id)
                )
    ...
    async def rules(ctx, channelID: int, buttonLabel: str, userMessage: str):
        ...
        if channel:
            if len(userMessage) < 2000:
                ...
                logger.info(
                    f"Can't send message is higher than 2000 character limit!\nMessage Content: {userMessage}"
                )
    ...
    async def roleassign(ctx, message: str, *roles_and_labels):
        ...
        add_roles_and_labels(
            roles_buttons_filename, message_id.id, roles_and_labels_parsed
        )
        await ctx.message.delete()
    ...
    @bot.slash_command(name="showmecat", description="Sends random cat image!")
    async def showmecat(ctx: discord.ApplicationContext):
